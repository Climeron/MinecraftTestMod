...
{
	public static final IBlockState LOG = InitBlocks.STEM.getDefaultState().withProperty(Stem.VARIANT, EnumHandler.EnumType.CRIMSON).withProperty(Stem.AXIS, EnumFacing.Axis.Y);
	...
	public boolean generate(World world, Random rand, BlockPos pos)
	{
		...
		if(!flag)
		{
			world.spawnEntity(new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(InitBlocks.FUNGUS, 1, 1)));
			return false;
		}
		else
		{
			...
			if(isSoil && y < world.getHeight() - height - 1)
			{
					...
					for(int xPos = x - b2; xPos <= x + b2 && flag; xPos++)
					{
						...
						for(int zPos = z - b2; zPos <= z + b2 && flag; zPos++)
						{
							...
							if(Math.abs(b3) != b2 || Math.abs(b4) != b2 || rand.nextInt(2) != 0 && b1 != 0)
							{
								...
								{
									this.setBlockAndNotifyAdequately(world, treePos, Blocks.NETHER_WART_BLOCK.getDefaultState());
								}
							}
						}			
					}
				...
				for (int logHeight = 0; logHeight < height; logHeight++)
				{
					...
					if (logState.getBlock().isAir(logState, world, up) ||
						...
						logState.getMaterial() == Material.VINE ||
				...
				for (int i = 0; i < 4; i++)	//Place vines
				{
					...
					if (rand.nextBoolean() == true)
					{
						...
						{
							this.setBlockAndNotifyAdequately(world, new BlockPos(up.getX() + deltaX, up.getY(), up.getZ() + deltaZ), InitBlocks.WEEPING_VINES.getDefaultState());
							if(rand.nextInt(3) > 0 && world.getBlockState(new BlockPos(up.getX() + deltaX, up.getY() - 1, up.getZ() + deltaZ)) == Blocks.AIR.getDefaultState())
							{
								this.setBlockAndNotifyAdequately(world, new BlockPos(up.getX() + deltaX, up.getY(), up.getZ() + deltaZ), InitBlocks.WEEPING_VINES.getDefaultState().withProperty(WeepingVines.VARIANT, EnumHandlerVines.EnumType.PLANT));
								this.setBlockAndNotifyAdequately(world, new BlockPos(up.getX() + deltaX, up.getY() - 1, up.getZ() + deltaZ), InitBlocks.WEEPING_VINES.getDefaultState());
								if(rand.nextInt(2) > 0 && world.getBlockState(new BlockPos(up.getX() + deltaX, up.getY() - 2, up.getZ() + deltaZ)) == Blocks.AIR.getDefaultState())
    							{
