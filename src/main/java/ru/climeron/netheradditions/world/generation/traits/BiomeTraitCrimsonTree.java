...
public class BiomeTraitCrimsonTree extends BiomeTraitAbstractTree
{
    protected IBlockState logBlock;
    protected IBlockState leafBlock;
    protected IBlockState blockToTarget;
    
    protected BiomeTraitCrimsonTree(Builder builder)
    {
        super(builder);
    ...
    public static BiomeTraitCrimsonTree create(Consumer<Builder> consumer)
    {
        ...
        if(pos.getY() > 0 && pos.getY() + height + 1 <= world.getHeight())
        {
            ...
            else
            {
                ...
                if(this.blockToTarget == checkState && pos.getY() < world.getHeight() - height - 1)
                {
                    ...
                    for (int i = 0; i < 4; i++)	//Place vines
    				{
    					int deltaX;
    					int deltaZ;
    					BlockPos up = pos.up(height - 4);
    					...
    					if (random.nextBoolean() == true)
    					{
    						...
    						if(world.getBlockState(new BlockPos(up.getX() + deltaX, up.getY() + 1, up.getZ() + deltaZ)) == Blocks.NETHER_WART_BLOCK.getDefaultState() && world.getBlockState(new BlockPos(up.getX() + deltaX, up.getY(), up.getZ() + deltaZ)) == Blocks.AIR.getDefaultState())
    						{
    							world.setBlockState(new BlockPos(up.getX() + deltaX, up.getY(), up.getZ() + deltaZ), InitBlocks.WEEPING_VINES.getDefaultState());
    							if(random.nextInt(3) > 0 && world.getBlockState(new BlockPos(up.getX() + deltaX, up.getY() - 1, up.getZ() + deltaZ)) == Blocks.AIR.getDefaultState())
    							{
    								world.setBlockState(new BlockPos(up.getX() + deltaX, up.getY(), up.getZ() + deltaZ), InitBlocks.WEEPING_VINES.getDefaultState().withProperty(WeepingVines.VARIANT, EnumHandlerVines.EnumType.PLANT));
    								world.setBlockState(new BlockPos(up.getX() + deltaX, up.getY() - 1, up.getZ() + deltaZ), InitBlocks.WEEPING_VINES.getDefaultState());
    								if(random.nextInt(2) > 0 && world.getBlockState(new BlockPos(up.getX() + deltaX, up.getY() - 2, up.getZ() + deltaZ)) == Blocks.AIR.getDefaultState())
        							{
        								world.setBlockState(new BlockPos(up.getX() + deltaX, up.getY() - 1, up.getZ() + deltaZ), InitBlocks.WEEPING_VINES.getDefaultState().withProperty(WeepingVines.VARIANT, EnumHandlerVines.EnumType.PLANT));
        								world.setBlockState(new BlockPos(up.getX() + deltaX, up.getY() - 2, up.getZ() + deltaZ), InitBlocks.WEEPING_VINES.getDefaultState());
        							}
    							}
    						}
    					}
    ...
    public static class Builder extends BiomeTraitAbstractTree.Builder
    {
        ...
        public Builder()
        {
            this.logBlock = InitBlocks.STEM.getDefaultState().withProperty(Nylium.VARIANT, EnumHandler.EnumType.CRIMSON);
            this.leafBlock = Blocks.NETHER_WART_BLOCK.getDefaultState();
            this.blockToTarget = InitBlocks.NYLIUM.getDefaultState().withProperty(Nylium.VARIANT, EnumHandler.EnumType.CRIMSON);
        }
        
        @Override
        public BiomeTraitCrimsonTree create()
        {
            return new BiomeTraitCrimsonTree(this);
        }
    }
}
